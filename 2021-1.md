...
看来还是 Vivado 我不会用啊，刚刚查资料才知道 ILA 可以设置触发条件，并不是从头开始抓波形的，然后我用了一上午学会了该如何调试，新技能 get

抓波形抓了半下午，发现 PC 一直在输出的函数里面，抓到最后终于找到问题出在哪了。PC 停在了这一指令处：

```
     2e0:	00052583          	lw	a1,0(a0) # 10800000 <_stack+0x107fec48>
```

这下子就能确定是 CFG Flash 的问题了

把 CFG Flash 所在的 AXI Quad Flash 的 XIP 关掉之后，BootLoader 成功输出了：
...
但是代码看起来也没什么问题啊，我给 BootLoader 加上输出 ELF 头的 Magic Number 之后显示，得到的 Magic Number 是 `0x0`，很迷惑

我怀疑是 AXI Quad SPI 的问题。这一 IP Core 有一个名为 `sck` 的信号，在我的板子上是属于 Bank 0 的，不知为什么设置管脚的时候设不上

在 NonTrivialMIPS 和原版 Fuxi 上 `sck` 这一信号也都没有直接使用，而是在 `.xdc` 文件中创建一个时钟连接，而且在我移植的 Fuxi SoC 中会报一大堆 Critical Warning，都报在这个 `.xdc` 文件里

把 BootLoader 的 `FLASH_ADDR` 改成 `0x10000000` 读一下试试。这个地址按道理是会烧进去 FPGA 的 `.bit` 文件的，如果独到的还是 `0x0` 那就是说明 AXI Quad SPI 有问题了

果然是 AXI Quad SPI 的问题，读不到东西

测试了一下，UART 在我的板子上用起来也有点问题。直接发送的话会丢包，在发送中间加一句 `time.sleep(0.001)` 给一点缓冲的时间就可以正常发送了

经过测试，HelloWorld 小程序启动成功
...
又试了一下，GeeOS 也启动成功了！不过不能在 shell 里输入，不知道是哪里出的问题
...
* 学会了用 ILA 调试板子上的信号
* 在 Fuxi SoC 上通过串口启动 GeeOS 成功

### Day 32 计划

1. 修改 `timing.xdc`，让 CFG Flash 可以正常工作
2. 在 Fuxi SoC 上让 GeeOS 通过 Flash 启动
