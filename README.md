...
我觉得去网上查也没什么用了，自己动手丰衣足食

折腾了好久，把 DDR3 对外输出的总信号拆分开，变成一个一个的，然后把单向 `output` 的都给 ILA 连上，这样应该就可以 debug 了吧，然而报错了...`[DRC REQP-127]` 和 `[DRC REQP-1581]`，意思是一个 obuf 只能驱动一个输出，不能再把信号传给 ILA 了 orz

有没有什么办法，既能让 DDR3 对外输出，又能抓到输出的信号内容呢

参考了前天找到的仿真 DDR3 的教程，亲测可用！而且复现了 `init_calib_complete` 拉不上去的问题！

今天有点累，前段时间熬夜太多了，今天先到这，早点休息了

### Day 26 进展

* 仿真 DDR3 成功，复现了 DDR3 无法初始化的问题
...
* `ddr3_ras_n` 基本全程处于 1，偶尔降回 0 又立刻拉上去
* `ddr3_cas_n` 开始时基本全程处于 1，偶尔降回 0 又立刻拉上去，中间有一小段像时钟一样频繁震荡，但 1 的时间大于 0 的时间
* `ddr3_we_n` 与 `ddr3_ras_n` 类似

就列这几个最明显的好了，其他的也有变化，但是变化不大。之后某一个时间，`init_calib_complete` 变成 1，以上三个信号就也一直是 1 了

此外，还有一件事！`sys_clk_i` 的频率一定要比 `clk_ref_i` 高！（20210128 深夜补充：**结论有误**）

我把 `sys_clk_i` 连到 `clk_ddr` 上，`clk_ref_i` 连到 `clk_peri` 上，初始化就可以进行到 `ddr3_cas_n` 反复震荡那一步了！

还有一件事！我的内存条到货了！现在跑 Vivado 终于不会闪退了！而且再也不用在综合的时候把浏览器关掉了

经过我的仔细观察，在 `ddr3_cas_n` 反复震荡的阶段，`ddr3_dq` 应该有一个固定的值，而不是保持 Z 不变，`ddr_dqs_p` 和 `ddr_dqs_n` 也应该在 `000` 和 `111` 之间反复震荡才对

折腾好久突然发现 `SoC.v` 里 DDR3 相关的信号位宽没有根据我的板子修改，Vivado 也不报个错，虽然改完之后并没有什么变化

### Day 28 进展

* 发现 Fuxi SoC 的 DDR3 两条时钟信号接反了
  * 更正：看了下 NonTrivialMIPS，似乎也是这么接的，并没有反，但是在我的板子上经过不完全的研究表明，`clk_ref_i` 比 `sys_clk_i` 快的话会导致无法初始化
  * 再更正：**研究有误，无视以上结论**，我再把连线改回来...
* 现在 DDR3 可以开始初始化了，但是还不能初始化完全
...
（20210130 补充：Fuxi SoC 中并不是遗漏了 `DDR3_cs_n` 信号，而是由于不明原因，ddr3\_controller 根本就没有这个信号）
...
亲测把 ELF 文件烧到 Flash 里启动不起来。我也检查了一下，代码是没有问题的，`fuxi.yu` 里的 `FLASH_ADDR` 是 `0x10800000`，烧写 `.mcs` 文件的时候偏移是 `0x00800000`，然而这样子不知道该怎么 debug，要不改天再仿真抓波形试试看吧，先用 UART 调试着先

但是这样子的话，只能启动 OpenSBI 本体，而不能用 SBI 启动操作系统，那我移植了个寂寞...而且 xv6 也不支持 UART 启动的功能，那咋移植嘛

看来 Flash 是绕不开了，一会再研究下 NonTrivialMIPS 的代码，看看 Fuxi SoC 不能读 Flash 的问题出在哪

然后...亲测串口启动不起来，我继续仿真折腾吧...

读 AXI Quad SPI 这一 IP Core 的 example 时偶然发现，Flash 的 IO2 和 IO3 是在 Quad 模式下才启用的，而 Standard 模式下并不需要。之前我手贱把 Standard 改成了 Quad 但是没有改 `SoC.v`，这才导致读不了 Flash，我马上改...

虽然改完也没正常运行，还是要仿真 debug 的

那么问题来了，怎么验证仿真条件下正确运行了呢？`addr` 变成 `0x80000000` 之后的地址？

然而 AXI Quad SPI 的 example 是 VHDL 写的，我有点看不懂，也不知道该怎么让这个模块读文件，因为这个 example 设置 `.mem` 文件的地方在主模块而不是仿真模块里，就很迷

仿真也太慢了，仿真进行到 1 ms，我这边已经过去了半个小时，看波形结合对 BootLoader 的 objdump 结果，可以看出连启动那句话都没输出完...

仿真可靠是可靠，但是太慢了啊，这条路行不通

有什么方法能在板子上抓波形呢，ILA 设置的深度大一点我的板子又承受不了，场面再次陷入僵局

要不再上 MicroBlaze 试试，MicroBlaze 对板子的要求不高，可以让 ILA 多抓一些

明天这样试试看吧

### Day 30 进展

* 发现了自己对 Flash 配置上的一些问题
* 发现我移植的 SoC 还是有问题，无论是 Flash 还是串口都启动不了 ELF 文件

### Day 31 计划

1. 用 MicroBlaze 研究 QSPI Flash 该怎么配置
2. 让 HelloWorld 小程序在 FPGA 上启动

额外计划: 这个仓库的 README 好像有点过长了，明天考虑一下改每个月写一份 Markdown，然后在 README 里放总结(?)
